...
# После кого тика будет постоянная разрядка АКБ
DISCHARGE_END_ENERGY = 15

# когда солнечки отрубаются
SUN_OFF_POINTS = {
    "s6": [15, 50, 75],
    "sB": [16, 51, 76],
}
...
class MyController:
    def __init__(self, psm):
        self.psm = psm
        self.accums = ["c1"]
        # self.prices = {"d8": 5.5, "h6": 5}
        self.prices = {"d1": 6.8, "h6": 5}
        self.table = "В"
        print([obj.address for obj in self.psm.objects])
        self.obj_types = [
            "main"  # подстанции
        ...
        self.path2net = {net.location: net for net in self.psm.networks.values()}

        self.station_names = {"main", "miniA", "miniB"}

        self.addr2obj = {obj.address: obj for obj in self.psm.objects}
    ...
    def objects_process(self):
        ...
        print("Криво косо", (consumption / psm.total_power.consumed) * psm.total_power.losses)
        return shortage
    ...
    def solar_process(self, obj):
        ...
        # zero_bold = 0.05
        if self.psm.tick in SUN_OFF_POINTS[obj.address[0]]:
            self.psm.orders.line_off(obj_st, line_num)
            return 0
        ...
        if psm.tick >= 80:
            ...
            if self.now_sun == 0:
                energy = obj.power.now.generated
            else:
    ...
    def run(self):
        ...
        if shortage > 0:
            if self.get_full_acb_charge() >= 99.9 * len(self.accums):
                self.psm.orders.sell(abs(shortage) * 0.8, 10)
            else:
                energy = abs(shortage) * 0.9
                self.charge_acbs(energy)
                if energy > 15:
                    self.psm.orders.sell((energy - 15) * 0.9, 10)
        elif shortage < 0:
            if self.get_full_acb_charge() < 0.1 * len(self.accums):
                self.psm.orders.buy(abs(shortage) * 1.1, 1)
            else:
                if abs(shortage) < 15:
                    self.discharge_acbs(abs(shortage) * 1.1)
                else:
                    self.discharge_acbs(15)
                    self.psm.orders.buy((abs(shortage) - 15) * 1.1, 1)
        ...
        for i, net in self.psm.networks.items():
            if net.location:
                if net.wear > CRITICAL_WEAR:
                    ...
                    CAN_OFF = True
                    for obj in endpoint2obj[location[-1]]:
        ...
        print(-self.get_losses(tick=self.psm.tick))
        # print(self.psm.orders.get())
        real_loss = 0
        real_loss = sum([net.losses for i, net in self.psm.networks.items() if i in (2, 3, 4)])
        # print(i, net)

        print(self.psm.total_power.losses)
    ...
    def get_predict(self, tick, obj):
        consumption = 0
        if obj.type.lower() == "housea":
            addr = obj.address[0]
            additional = 0.82 * (5 - obj.contract) ** 2.6 if obj.contract < 5 else 0
            consumption = psm.forecasts.houseA[tick] + additional + 0.5
            consumption = obj.power.now.consumed
            return -consumption
        if obj.type.lower() == "houseb":
            addr = obj.address[0]
            additional = 0.24 * (9 - obj.contract) ** 2.2 if obj.contract < 8 else 0
            consumption = psm.forecasts.houseB[tick] + additional + 0.5
            consumption = obj.power.now.consumed
            return -consumption
        if obj.type == "storage":
            energy = 0
            for order in self.psm.orders.get():
                if order["orderT"] == "discharge":
                    if order["address"] == obj.address[0]:
                        energy = order["power"]
                elif order["orderT"] == "charge":
                    if order["address"] == obj.address[0]:
                        energy = -order["power"]
            charge = obj.charge.now
            if energy > 0:
                energy = min(charge, abs(energy))
            return energy
        if obj.type == "factory":
            path1 = obj.path[0]
            path2 = obj.path[1]
            access1 = all([self.path2net[(path1[:i + 1])].online for i in range(len(path1))])
            access2 = all([self.path2net[(path2[:i + 1])].online for i in range(len(path2))])
            consumption = psm.forecasts.factory[tick] + 0.5
            consumption = obj.power.now.consumed
            if access1 and access2:
                return -consumption / 2
            elif access1 or access2:
                return -consumption
            else:
                return 0
        ...
        if obj.type == "hospital":
            ...
            access1 = all([self.path2net[(path1[:i + 1])].online for i in range(len(path1))])
            access2 = all([self.path2net[(path2[:i + 1])].online for i in range(len(path2))])
            consumption = psm.forecasts.hospital[tick] + 0.5
            consumption = obj.power.now.consumed
            if access1 and access2:
                return -consumption / 2
            elif access1 or access2:
        ...
        if obj.type == "solar":
            ...
            corr_next_sun = max(self.psm.forecasts.sun[tick], 0)
            if self.now_sun == 0:
                energy = obj.power.now.generated
            else:
                energy = obj.power.now.generated * (corr_next_sun / self.now_sun)
    ...
    def get_energy_loss(self, energy):
        energy = abs(energy)
        coef_loss = ((energy / 50) ** 2.4) * 0.6
        return coef_loss

    def get_wear_loss(self, wear):
        return (wear ** 2) * 0.5

    def get_wear(self, energy):
        w = ((abs(energy) / 50) ** 1.9) / 2
        return w

    def full_loss(self, wear, energy):
        energy = abs(energy)
        return (1 - (-self.get_wear_loss(wear) + 1) * (-self.get_energy_loss(energy) + 1)) * energy

    def get_losses(self, tick, st_obj=None):
        if st_obj is None:
            st_obj = [obj for obj in self.psm.objects if obj.type == "main"][0]

        num_lines = 2 if st_obj.type == "miniB" else 3

        total_loss = 0
        total_power = 0
        for i in range(1, num_lines + 1):
            line_obj = ips.Line(id=st_obj.id, line=i)
            if not (line_obj in self.endpoint2obj.keys()):
                continue
            # print(st_obj.path[0])
            location = tuple(list(st_obj.path[0]) + [line_obj])
            net = self.path2net[location]

            if not net.online:
                continue

            line_energy = 0
            for obj in self.endpoint2obj[line_obj]:
                if obj.type in self.station_names:
                    line_energy += self.get_losses(tick, obj)
                else:
                    line_energy += self.get_predict(tick, obj)
                    print(line_energy, obj.address, self.full_loss(net.wear, self.get_predict(tick, obj)))

            total_power += line_energy - self.full_loss(net.wear, line_energy)
            total_loss += - self.full_loss(net.wear, line_energy)
