...
sell_price = 5
buy_price = 2
lishn = psm.total_power.generated - psm.total_power.consumed - 3
if lishn > 0:
...
for index, net in psm.networks.items():
    ...
    if ips_past.Line(('miniA', 1), 3) in net.location:
        line_21_wear = net.wear
    if ips_past.Line(('miniB', 2), 1) in net.location:
        line_22_wear = net.wear
...
if psm.tick < 5:
    psm.orders.line_off("e1", 3)
else:
    wall = 0.4
    if line_21_wear < wall and line_22_wear < wall:
        pass
    elif line_21_wear > wall:
        psm.orders.line_off("e1", 3)
    elif line_22_wear > wall:
        psm.orders.line_off("m2", 1)
...
if shortage > 0:
    ...
    print(
        f"В ТЭС tJ загружено {fu} и получено {get_tes_power(fu, last_energy)} МВт и цена за это {fu * 3.5} руб",
        f"На прошлом ходу было произведено {last_energy}МВт")
    psm.orders.tps("tJ", fu)

if shortage > 0:
    fu = get_num_fumes(shortage)
    print(
        f"В ТЭС tO загружено {fu} и получено {get_tes_power(fu, last_energy)} МВт и цена за это {fu * 3.5} руб",
        f"На прошлом ходу было произведено {last_energy}МВт")
    psm.orders.tps("tO", fu)
